...
user_problem_statement: |
  НОВЫЙ ЗАПРОС НА ТЕСТИРОВАНИЕ: Протестировать работу фильтров в приложении SecuRisk
  
  Цель теста: Проверить что фильтры начинают работать сразу после выбора значения (без нажатия дополнительных кнопок).
  
  Разделы для тестирования:
  1. Инциденты (Incidents) - фильтры: Статус, Критичность, Общий поиск, Сброс фильтров
  2. Активы (Assets) - фильтры: Статус, Критичность, Общий поиск, Сброс фильтров  
  3. Риски (RiskRegister) - фильтры: Категория, Статус, Уровень риска, Общий поиск, Сброс фильтров
  
  Требования: Фильтры должны работать МГНОВЕННО при изменении значения, без нажатия кнопок "Применить"
